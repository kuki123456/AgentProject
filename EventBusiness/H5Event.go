...
func H5EventBusiness(H5Eventbody string,requestbody string,file io.Writer){
	...
	//pvid页面id
	Utils.AssertType(gjson.Get(H5Eventbody,"v.pvid").Type,3,file,gjson.Get(H5Eventbody,"v.pvid").Value(),"v.pvid",H5Eventbody,"H5Event")
   //url,request请求地址url
	Utils.AssertType(gjson.Get(H5Eventbody,"v.url").Type,3,file,gjson.Get(H5Eventbody,"v.url").Value(),"v.url",H5Eventbody,"H5Event")
	//"wpi":{},//WebviewPerformanceInfo 页面性能数据 [非空字段]
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi").Type,5,file,gjson.Get(H5Eventbody,"v.wpi").Value(),"v.wpi",H5Eventbody,"H5Event")
	//{
	//	"ns":1,//navigation start [时间戳] (表征了从同一个浏览器上下文的上一个文档卸载(unload)结束时的UNIX时间戳。如果没有上一个文档，这个值会和PerformanceTiming.fetchStart相同)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.ns").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.ns").Value(),"v.wpi.ns",H5Eventbody,"H5Event")
	...
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.ues").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.ues").Value(),"v.wpi.ues",H5Eventbody,"H5Event")
	...
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.uee").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.uee").Value(),"v.wpi.uee",H5Eventbody,"H5Event")
	//    "rds":1,//redirect start [相对值] (表征了第一个HTTP重定向开始时的UNIX时间戳。如果没有重定向，或者重定向中的一个不同源，这个值会返回0)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.rds").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.rds").Value(),"v.wpi.rds",H5Eventbody,"H5Event")
	//    "rde":1,//redirec end [相对值] (表征了最后一个HTTP重定向完成时（也就是说是HTTP响应的最后一个比特直接被收到的时间）的UNIX时间戳。如果没有重定向，或者重定向中的一个不同源，这个值会返回0)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.rde").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.rde").Value(),"v.wpi.rde",H5Eventbody,"H5Event")
	//    "fs":1,//fetch start [相对值] (表征了浏览器准备好使用HTTP请求来获取(fetch)文档的UNIX时间戳。这个时间点会在检查任何应用缓存之前)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.fs").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.fs").Value(),"v.wpi.fs",H5Eventbody,"H5Event")
	...
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dls").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dls").Value(),"v.wpi.dls",H5Eventbody,"H5Event")
	...
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dle").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dle").Value(),"v.wpi.dle",H5Eventbody,"H5Event")
	//    "cs":1,//connect start [相对值] (返回HTTP请求开始向服务器发送时的Unix毫秒时间戳。如果使用持久连接（persistent connection），则返回值等同于fetchStart属性的值)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.cs").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.cs").Value(),"v.wpi.cs",H5Eventbody,"H5Event")
	//    "scs":1,//secure connection start [相对值] (返回浏览器与服务器开始安全链接的握手时的Unix毫秒时间戳。如果当前网页不要求安全连接，则返回0)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.scs").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.scs").Value(),"v.wpi.scs",H5Eventbody,"H5Event")
	//    "ce":1,//connect end [相对值] (返回浏览器与服务器之间的连接建立时的Unix毫秒时间戳。如果建立的是持久连接，则返回值等同于fetchStart属性的值。连接建立指的是所有握手和认证过程全部结束)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.ce").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.ce").Value(),"v.wpi.ce",H5Eventbody,"H5Event")
	//    "reqs":1,//request start [相对值] (返回浏览器向服务器发出HTTP请求时（或开始读取本地缓存时）的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.reqs").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.reqs").Value(),"v.wpi.reqs",H5Eventbody,"H5Event")
	//    "rsps":1,//response start [相对值] (返回浏览器从服务器收到（或从本地缓存读取）第一个字节时的Unix毫秒时间戳。如果传输层在开始请求之后失败并且连接被重开，该属性将会被数制成新的请求的相对应的发起时间)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.rsps").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.rsps").Value(),"v.wpi.rsps",H5Eventbody,"H5Event")
	//    "rspe":1,//response end [相对值] (返回浏览器从服务器收到（或从本地缓存读取，或从本地资源读取）最后一个字节时（如果在此之前HTTP连接已经关闭，则返回关闭时）的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.rspe").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.rspe").Value(),"v.wpi.rspe",H5Eventbody,"H5Event")
	//    "dl":1,//dom loading [相对值] (返回当前网页DOM结构开始解析时（即Document.readyState属性变为"loading"、相应的 readystatechange事件触发时）的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dl").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dl").Value(),"v.wpi.dl",H5Eventbody,"H5Event")
	...
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.di").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.di").Value(),"v.wpi.di",H5Eventbody,"H5Event")
	//    "dcles":1,//dom content loaded event start [相对值] (返回当解析器发送DOMContentLoaded 事件，即所有需要被执行的脚本已经被解析时的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dcles").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dcles").Value(),"v.wpi.dcles",H5Eventbody,"H5Event")
	//    "dclee":1,//dom content loaded event end [相对值] (返回当所有需要立即执行的脚本已经被执行（不论执行顺序）时的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dclee").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dclee").Value(),"v.wpi.dclee",H5Eventbody,"H5Event")
	//    "dc":1,//dom complete [相对值] (返回当前文档解析完成，即Document.readyState 变为 'complete'且相对应的readystatechange 被触发时的Unix毫秒时间戳)
	Utils.AssertType(gjson.Get(H5Eventbody,"v.wpi.dc").Type,2,file,gjson.Get(H5Eventbody,"v.wpi.dc").Value(),"v.wpi.dc",H5Eventbody,"H5Event")
