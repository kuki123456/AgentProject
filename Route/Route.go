...
type postback struct {
	...
	Casebody string `json:"casebody"`
	Casemoudle string `json:"casemoudle"`
}
...
func ReturninputMap(input int)[]map[string]interface{} {
	...
	case 14:
		...
		return []map[string]interface{}{{"n":"routechange","c":100},{"c":100,"n":"network"},{"c":100,"n":"h5"},{"c":100,"n":"view","tv":200},{"c":100,"n":"crash"},{"c":100,"n":"coollaunch","tv":500},{"c":100,"n":"hotlaunch","tv":200},{"c":100,"n":"action","tv":3000},{"c":100,"n":"statechange"},{"c":100,"n":"anr"},{"c":100,"n":"customlog"},{"c":100,"n":"customevent"},{"c":100,"n":"custommetric"},{"c":100,"n":"lagfps","tv":59},{"c":100,"n":"lagstuck","tv":6}}
		//return []map[string]interface{}{{"c":100,"n":"custommetric"},{"c":100,"n":"custommetric"},{"c":100,"n":"customevent"},{"c":100,"n":"crash"},{"c":100,"n":"view","tv":200},}
		default:
		log.Println("输入有误！")
	}
	return nil
...
var casename = ""
var moudle = ""
var session= "dfewgwghoiwjgvvwjgjwkgmw"
func Route(){
	// 创建一个默认的路由引擎
	r := gin.Default()
	r.LoadHTMLFiles("./Route/custom-service.html")
	...
	r.POST("/config",func(c *gin.Context){
		...
		cpByte,_:=json.Marshal(cp)
		cp_json:=string(cpByte)
		if gjson.Get(RESPONSE,"s").Exists(){
			session=gjson.Get(RESPONSE,"s").String()
		}
	...
	r.POST("/upload",func(c *gin.Context){
		...
		body,_ := ioutil.ReadAll(c.Request.Body)
		result,_:= ParseGzip(body)
		err:=json.Unmarshal(result,&requestBody)
		if err!=nil{
		...
		casedata:=postback{
			Casename: casename,
			Casebody: string(result),
			Casemoudle: moudle,
		}
		casedata_byte,_:=json.Marshal(casedata)
		Utils.POSTCOMMON("http://127.0.0.1:8000/postback",strings.NewReader(string(casedata_byte)))
		//Utils.WriteCaseDate(casename,moudle,string(result))
		...
		paramByte,_:=json.Marshal(paramer)
		Utils.POSTCOMMON("http://127.0.0.1:8000/upload",strings.NewReader(string(paramByte)))
		fmt.Println("******************************sdk版本*****************************")
		...
		for _,value:=range eventbody{
			...
			if gjson.Get(string(result),"usd").Num >3600000000 {
				_, _ = fmt.Fprintf(file, "%v[ERROR]:[UPLOAD]的usd异常！\n", time.Now().Format("2006/01/02 15:04:05"))
	...
	r.GET("/caseinfo", func(context *gin.Context) {
		casename = context.Query("name")
		moudle = context.Query("moudle")
		context.JSON(http.StatusOK,gin.H{"code":200,"message":fmt.Sprintf("%s and %s:caseinfo更新成功！",casename,moudle)})
	})
	c := cron.New()
	spec := "00 30 23 * *  "
